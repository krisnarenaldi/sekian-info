/**
 * POST /api/cron/news-sport — Pipeline "Berita Sport"
 *
 ...
 *   → LLM Summarize → Save ke daily_digest dengan feed_type='sport'
 *
...
import { fetchAllSources, SPORT_NEWS_SOURCES } from '@/lib/pipeline/rss-fetcher'
...
const log = createLogger('cron-news-sport')
 ...
 * Contoh: "Timnas Indonesia Kalahkan Vietnam" → "timnas-indonesia-kalahkan-vietnam"
 */
...
export async function POST(req: NextRequest): Promise<NextResponse> {
  ...
  if (!expectedToken || authHeader !== `Bearer ${expectedToken}`) {
    log.warn('Unauthorized request to /api/cron/news-sport')
    return NextResponse.json({ error: 'Unauthorized' }, { status: 401 })
  }

  const date = todayISO()
  log.info(`Pipeline "Berita Sport" dimulai untuk tanggal ${date}`)
  ...
  try {
    ...
    log.info('Step 1/6: Fetching RSS dari semua sumber berita olahraga')
    const rawItems = await fetchAllSources(SPORT_NEWS_SOURCES)
    log.info(`Step 1/6: Berhasil mengambil ${rawItems.length} item dari RSS`)

    if (rawItems.length === 0) {
      log.warn('Tidak ada artikel yang berhasil di-fetch dari semua sumber RSS olahraga')
      return NextResponse.json(
        { error: 'Semua sumber RSS olahraga tidak dapat diakses. Data hari sebelumnya dipertahankan.' },
        { status: 503 },
      )
    }
    ...
    try {
      ...
      log.error(
        'LLM_Summarizer gagal pada pipeline "Berita Sport". ' +
    ...
    // ── 8. Save ke daily_digest dengan feed_type='sport' ─────────────────────
    log.info(`Menyimpan ${newsItems.length} item ke tabel daily_digest dengan feed_type='sport' (tanggal: ${date})`)
    ...
    const result = await upsertDailyDigest(date, digestItems, 'sport')
    ...
    log.info(`Pipeline "Berita Sport" selesai. ${result.count} item tersimpan untuk tanggal ${date}.`)
    ...
    return NextResponse.json({
      ...
      feedType: 'sport',
      message: `Pipeline berhasil. ${result.count} berita olahraga tersimpan untuk ${date}.`,
    })
  } catch (err) {
    // Tangkap error yang tidak terduga dari tahap non-LLM (embedding, cluster, dll.)
    log.error('Pipeline "Berita Sport" gagal dengan error tidak terduga', err)
    return NextResponse.json(
      {
