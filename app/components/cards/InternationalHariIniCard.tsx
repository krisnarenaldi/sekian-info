...
function renderListItem(item: DailyDigestRow, badgeClass: string) {
  ...
  return (
    <li key={item.id}>
      <Link href={`/news/${item.slug}`} className="group block px-5 py-3.5 hover:bg-gray-50 dark:hover:bg-gray-700/40 transition-colors focus:outline-none focus-visible:ring-2 focus-visible:ring-indigo-500 focus-visible:ring-inset" aria-label={`Baca lebih lanjut tentang ${topicLabel}`}>
        <p className="text-sm font-semibold text-gray-800 dark:text-gray-100 leading-snug group-hover:text-indigo-600 dark:group-hover:text-indigo-400 transition-colors">{topicLabel}</p>
  ...
  return (
    ...
    >
      ...
      <div className="absolute bottom-0 left-0 right-0 p-4">
        {item.category && (
          <span className="inline-block text-[10px] font-semibold px-1.5 py-0.5 rounded mb-2 text-white bg-indigo-500/80">
            {item.category}
          </span>
        )}
...
export default function InternationalHariIniCard({ digestItems }: Props) {
  ...
  return (
    ...
    >
      <div className="flex items-center justify-between px-5 py-3.5 border-b border-gray-100 dark:border-gray-700 bg-[#e4e4e7]">
        <h2 id="international-heading" className="text-sm font-bold text-gray-800 dark:text-gray-100 flex items-center gap-2">
          <span aria-hidden="true">🌍</span> International Info
        </h2>
      ...
      {/* First item (index 0) */}
      {displayItems[0] && (
        <ul className="divide-y divide-gray-50 dark:divide-gray-700/50" aria-label="Daftar topik berita internasional hari ini">
          {renderListItem(displayItems[0], badgeClass)}
        </ul>
      )}

      {/* Hero image for second item (index 1) — appears between items 0 and 2 */}
      {renderHeroImage(displayItems[1])}

      {/* Items 2, 3, 4 */}
      {displayItems.slice(2).length > 0 && (
        <ul className="divide-y divide-gray-50 dark:divide-gray-700/50">
          {displayItems.slice(2).map((item) => renderListItem(item, badgeClass))}
        </ul>
      )}

      <div className="px-5 py-3 border-t border-gray-100 dark:border-gray-700 bg-gray-50/50 dark:bg-gray-700/20">
        <Link href="/news?feed_type=international" className="text-xs font-medium text-indigo-600 dark:text-indigo-400 hover:text-indigo-800 dark:hover:text-indigo-300 focus:outline-none focus-visible:ring-2 focus-visible:ring-indigo-500 rounded transition-colors" aria-label="Lihat daftar lengkap berita internasional hari ini">Lihat semua berita →</Link>
      </div>
    </section>
  )
}